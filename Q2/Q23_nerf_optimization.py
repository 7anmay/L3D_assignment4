...
def optimize_nerf(
    ...
    for epoch in range(max_epoch):
        ...
        for data in train_loader:
            ...
            pred_rgb = pred_rgb.resize(512, 512)
            print("pred_rgb  resized  ", pred_rgb.shape)
            latents = sds.encode_imgs(pred_rgb) 

            if not args.view_dep_text:
                loss = sds.sds_loss(latents, text_cond, text_uncond, guidance_scale=100, grad_scale=1)
            else:
                loss_front = sds.sds_loss(latents, text_front, text_uncond)
                loss_side = sds.sds_loss(latents, text_side, text_uncond)
                loss_back = sds.sds_loss(latents, text_back, text_uncond)
