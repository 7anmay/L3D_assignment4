...
class Gaussians:
    ...
    def __init__(
        ...
        if self.use_spherical_harmonics and data.get("spherical_harmonics") is not None:
            print("Loading Spherical Harmonics")
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            ...
            ### YOUR CODE HERE ###
            # implementing eqaution --> Σ = RSS^TR^T
            # R = I = torch.eye(3).to(self.device)
    ...
    def compute_cov_2D(
        ...
        # using equation 5 from the paper. --> Σ_2D = JWΣW^TJ^T
        # (N, 2, 2)
        T = torch.matmul(J,W)
        cov_2D = torch.matmul(T,torch.matmul(cov_3D, T.transpose(1,2)))
    ...
    def compute_means_2D(means_3D: torch.Tensor, camera: PerspectiveCameras):
        ...
        # print("means 3D shape ->",  means_3D.shape)
        means_2D = camera.transform_points_screen(means_3D)[:, :2]
        # print("means 2D shape ->",means_2D.shape)
        # print(means_2D)
        return means_2D
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        # z_vals = 1/self.gaussians.means[..., -1]  # (N,)
         # Get the means of 3D Gaussians
        means_3D = self.gaussians.means

        # Project 3D Gaussians to 2D screen space
        projected_points_3D = camera.get_world_to_view_transform().transform_points(means_3D)

        # Extract depth values (z coordinate)
        z_vals = projected_points_3D[:, 2]
    ...
    def get_idxs_to_filter_and_sort(self, z_vals: torch.Tensor):
        ...
        """
        ### YOUR CODE HERE ###
        # Sort the depth values in ascending order
        sorted_idxs = torch.argsort(z_vals)
        # print("sorted_idxs",sorted_idxs)
        # Remove invalid Gaussians with depth value less than 0
        valid_idxs = sorted_idxs[z_vals[sorted_idxs] >= 0]
        # print("valid_idxs",valid_idxs)
        # Return the sorted and valid indices
        idxs = valid_idxs
    ...
    def compute_alphas(self, opacities, means_2D, cov_2D, img_size):
        ...
        # print("exp_power",exp_power.shape)
    ...
    def compute_transmittance(
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation.
        #transmittance = torch.ones((len(one_minus_alphas)-1, H, W), device=alphas.device, dtype=alphas.dtype)
    ...
    def render(
        ...
        if not self.gaussians.use_spherical_harmonics:
        ...
        if self.gaussians.use_spherical_harmonics:
    ...
    def calculate_gaussian_directions(self, means_3D, camera):
        ...
        camera_position = camera.get_camera_center()  # (1, 3)
        direction_vectors = means_3D - camera_position  # (N, 3)
        gaussian_dirs = direction_vectors / direction_vectors.norm(dim=-1, keepdim=True)
